...
require("packer").startup(function(use)
    ...
    -- LSPインストーラー(:LspInstall :LspInstallinfo コマンドでlsをインストールする see:github, omnisharp-roslynも入る)
    ...
    use("udalov/kotlin-vim") -- neovimがkotlinのファイルタイプを認識しないために必要
    ...
    lsp_config.hls.setup({
        haskell = {
            formattingProvider = "fourmolu", -- null-ls側のサポートに合わせる
        },
    })
    lsp_config.html.setup({
        provideFormatter = false, -- formatterはprettierを使用
    })
    ...
    null_ls.setup({
        sources = {
            ...
            }), -- for javascript/typescript/react/vue, lsp版を使用しない理由はdiagnostics.eslint_d参照
            -- null_ls.builtins.code_actions.shellcheck, -- for bash, 使用しない理由はdiagnostics.shellcheck参照
            -- diagnostics
            ...
            null_ls.builtins.diagnostics.eslint_d.with({
                prefer_local = "node_modules/.bin",
            }), -- for javascript/typescript/react/vue, eslint-lspもあるがまだ開発中
            null_ls.builtins.diagnostics.flake8.with({
                prefer_local = ".venv/bin",
            }), -- for python
            -- null_ls.builtins.diagnostics.golangci_lint, -- for go, lsp版を使用する
            ...
            -- null_ls.builtins.diagnostics.rubocop, -- for ruby, 使用しない理由はformatting.rubocop参照
            -- null_ls.builtins.diagnostics.shellcheck, -- for bash, bashlsで使用される
            null_ls.builtins.diagnostics.stylelint.with({
                prefer_local = "node_modules/.bin",
            }), -- for css, eslint-lspに切り替えるなら合わせてstylelint-lspにしたほうが良い
            null_ls.builtins.diagnostics.sqlfluff.with({
                extra_args = { "--dialect", "postgres" },
            }), -- for sql, sqlsを使用しない理由はformatting.sqlfluff参照
            null_ls.builtins.diagnostics.tidy.with({
                prefer_local = "node_modules/.bin",
            }), -- for html/xml
            -- null_ls.builtins.diagnostics.tsc.with({
            -- 	prefer_local = "node_modules/.bin",
            -- }), -- for typescript/react, tsserverを使用する
            ...
            -- c/cppはcclsで使用される, cs/javaはominisharp/jdtlsのformattingを使用する
            -- null_ls.builtins.formatting.dart_format, -- for dart, dartlsで使用される
            -- null_ls.builtins.formatting.fourmolu, -- for haskell, hlsで使用される
            ...
            -- null_ls.builtins.formatting.rubocop, -- for ruby, solargraphで使用される
            -- null_ls.builtins.formatting.rustfmt, -- for rust, rust_analyzerで使用される
            null_ls.builtins.formatting.shfmt, -- for bash
            null_ls.builtins.formatting.sqlfluff.with({
                extra_args = { "--dialect", "postgres" },
            }), -- for sql, db定義補完はsqlsに切替(db接続必須, まだ開発途中), デフォルトはposgresだがpwdに.sqlfluffを配置すれば設定できる
            -- null_ls.builtins.formatting.stylua, -- for lua, sumneko_luaを使用
            -- null_ls.builtins.formatting.tidy.with({
            -- 	prefer_local = "node_modules/.bin",
            -- 	disabled_filetypes = { "html" },
            -- }), -- for xml, lemminxのformattingを使用する
