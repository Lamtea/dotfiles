...
local jdtls_path = path.get_install_path("jdtls")
...
local java_debug_paths = vim.fn.glob(
    path.get_install_path("java-debug-adapter") .. "/extension/server/com.microsoft.java.debug.plugin-*.jar"
)
local java_test_paths = vim.fn.glob(path.get_install_path("java-test") .. "/extension/server/*.jar")
local bundles = {
    java_debug_paths,
}
